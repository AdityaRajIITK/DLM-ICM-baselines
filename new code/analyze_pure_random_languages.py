"""
Analyze pure-random baseline output by language and write PNG plots to figures_pure_random.

Inputs:
  pure_random_structures.csv

Outputs:
  figures_pure_random/availability.csv
  figures_pure_random/by_language/{lang}_sentence_level_stats.csv
  figures_pure_random/by_language/{lang}_dependency_length_by_sentence_length.png
  figures_pure_random/by_language/{lang}_intervener_complexity_by_sentence_length.png
  figures_pure_random/by_language/{lang}_tree_depth_by_sentence_length.png
  figures_pure_random/by_language/{lang}_tree_density_by_sentence_length.png
  figures_pure_random/by_language/{lang}_max_arity_by_sentence_length.png
  figures_pure_random/by_language/{lang}_high_arity_nodes_ge3_by_sentence_length.png
"""
...
INPUT = Path(os.environ.get("PURE_RANDOM_INPUT", "pure_random_structures.csv"))
SUD_DIR = Path("SUD")
OUT_DIR = Path(os.environ.get("PURE_RANDOM_FIGURES_DIR", "figures_pure_random"))
LANG_OUT_DIR = OUT_DIR / "by_language"
HIGH_ARITY_THRESHOLD = 3
ANALYSIS_MODE = os.environ.get("PURE_RANDOM_ANALYSIS_MODE", "root0").strip().lower()
...
LABELS = {
    "real": "Real trees",
    "random": os.environ.get("PURE_RANDOM_LABEL", "Pure random trees"),
}
...
def available_sud_files() -> dict[str, Path | None]:
...
def read_sentence_rows() -> list[dict[str, int | float | str]]:
    if not INPUT.exists():
        raise SystemExit(
            f"{INPUT} not found. Run `python \"new code\\construct_pure_random_structures.py\"` first."
        )
...
def summarize_by_length(rows: list[dict[str, int | float | str]]):
...
def write_availability(sud_files: dict[str, Path | None], sentence_rows) -> None:
    OUT_DIR.mkdir(exist_ok=True)
    row_counts = Counter(str(row["lang"]) for row in sentence_rows)
    with (OUT_DIR / "availability.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "sud_train_file",
                "sentence_level_rows_in_pure_random_structures",
            ],
        )
        writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    "sentence_level_rows_in_pure_random_structures": row_counts[lang],
                }
            )


def write_language_stats(lang: str, rows) -> None:
    LANG_OUT_DIR.mkdir(parents=True, exist_ok=True)
    with (LANG_OUT_DIR / f"{lang}_sentence_level_stats.csv").open(
...
def plot_measure(lang: str, rows, *, measure: str, ylabel: str, title: str, outfile: Path) -> None:
...
def plot_language(lang: str, rows) -> None:
    LANG_OUT_DIR.mkdir(parents=True, exist_ok=True)
    plot_measure(
        lang,
        rows,
    ...
    plot_measure(
        lang,
        rows,
        measure="mean_tree_depth",
        ylabel="Mean tree depth",
        title="tree depth by sentence length",
        outfile=LANG_OUT_DIR / f"{lang}_tree_depth_by_sentence_length.png",
    )
    plot_measure(
        lang,
        rows,
    ...
    plot_measure(
        ...
        title="high-arity nodes by sentence length",
        outfile=LANG_OUT_DIR / f"{lang}_high_arity_nodes_ge3_by_sentence_length.png",
    )


def main() -> None:
    print(f"Reading {INPUT} and writing plots to {OUT_DIR}")
    sud_files = available_sud_files()
    ...
    print(f"Wrote {OUT_DIR / 'availability.csv'}")
    if plotted:
        print("Generated per-language analyses for: " + ", ".join(plotted))
    else:
        print(f"No rows were found in {INPUT}.")
